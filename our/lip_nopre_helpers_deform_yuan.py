...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, embedded_pts, num_slots=None):
        ...
        #inputs=np.concatenate((inputs.cpu().detach().numpy(),embedded_pts, max_rel.cpu().detach().numpy()),axis=-1)
...
def make_indices(pts, attention_poses, intrinsic, H, W):
    hom_points = np.concatenate((pts.cpu().numpy(), np.broadcast_to(torch.tensor([1.0]).cpu().numpy(), pts.shape[:-1] + (1,))),axis=-1)
    hom_points=torch.from_numpy(hom_points).cuda()
    #hom_points = torch.cat((pts, torch.broadcast_to(torch.tensor([1.0]), pts.shape[:-1] + (1,))), -1)
    extrinsic = invert(attention_poses)[:, :3]
    focal = intrinsic[0, 0]

    pt_camera = np.broadcast_to(hom_points[None, ...].cpu().numpy(),
                                (extrinsic.shape[0], hom_points.shape[0], hom_points.shape[1])) @ extrinsic.permute(0, 2, 1).cpu().numpy()
    pt_camera=torch.from_numpy(pt_camera).cuda()
...
class Face_Feature_NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, dim_aud=76,
        ...
        input_ch_all = input_ch+ dim_image_features+256
        self.pts_linears = nn.ModuleList(
            [nn.Linear(input_ch_all, W)] + [nn.Linear(W, W) if i not in self.skips else nn.Linear(W + input_ch_all, W) for i in range(D-1)])
    ...
    def forward(self, x):
        input_pts, input_views = torch.split(
            x, [self.input_ch+self.dim_image_features+256, self.input_ch_views], dim=-1)

        #print("x",x.shape)57600 666
        h = input_pts
        #print("h",h.shape)57600 639
        for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[i](h)
            h = F.relu(h)
            if i in self.skips:
...
class nerf_attention_model(nn.Module):
    ...
    def forward(self, inputs, training=None):
        ...
        pts = inputs[3]
        G_feature=G_feature.expand( local1.shape[1],G_feature.shape[-1])

        embedded_pts = nerf_inputs[...,:self.embed_ln]
        embedded_pts = embedded_pts.unsqueeze(0).expand(local1.shape[0], local1.shape[1], embedded_pts.shape[-1])
        #embedded_pts = np.broadcast_to(embedded_pts[None].cpu().detach().numpy(), (local1.shape[0], local1.shape[1], embedded_pts.shape[-1]) )
        
        #print(embedded_pts.shape)4 54400 63
        #attention_outputs = self.slot_att(local.permute(1,0,2), embedded_pts.permute(1,0,2))
        #attention_outputs1 = self.slot_att(np.transpose(local1.cpu().detach().numpy(),(1,0,2)),np.transpose(embedded_pts,(1,0,2)))

        attention_outputs1 = self.slot_att(local1.permute(1,0,2),embedded_pts.permute(1,0,2))
        #attention_outputs2 = self.slot_att2(G_feature.permute(1,0,2),embedded_pts.permute(1,0,2))
        #attention_outputs1 = torch.Tensor(attention_outputs1)
 
        decoder_input = torch.cat((attention_outputs1,G_feature, nerf_inputs), -1)
...
class Position_warp(nn.Module):
    ...
    def forward(self, query_pts, aud, reference_feature):
        aud=np.broadcast_to(aud.cpu().numpy(),(query_pts.shape[0],64))
        input=np.concatenate((query_pts.cpu().numpy(),aud,reference_feature.cpu().numpy()),axis=1)
