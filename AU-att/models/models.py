...
class BaseModel(object):
    ...
    def _save_optimizer(self, optimizer, optimizer_label, epoch_label):
        save_filename = 'opt_epoch_%s_id_%s.pth' % (100, optimizer_label)
    ...
    def _save_network(self, network, network_label, epoch_label):
        ...
        #save_filename = 'net_epoch_%s_id_%s.tar' % (epoch_label, network_label)
        save_filename = 'net_epoch_%s_id_%s.tar' % (100, network_label)
        save_path = os.path.join(self._save_dir, save_filename)
        torch.save({
            ...
            'unet_state_dict2': network.state_dict(),
        ...
        save_filename = 'net_epoch_%s_id_%s.pth' % (100, network_label)
        save_path = os.path.join(self._save_dir, save_filename)
