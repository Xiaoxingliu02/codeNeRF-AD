...
class GANimation(BaseModel):
    ...
    def set_input(self, input,i_epoch,s):
        self.count_mask=i_epoch
        self.count_s=s
    ...
    def forward(self, keep_data_for_visuals=False, return_estimates=False):
        if not self._is_train:
            ...
            if keep_data_for_visuals:
                ...
                filename = os.path.join('/fs1/home/tjuvis_2022/lxx/GANimation-master/png/',
                                        self.count_s+'mask{:03d}.png'.format(self.count_mask),)
                img = cv2.cvtColor(self._vis_fake_img_mask, cv2.COLOR_RGB2BGR)
    ...
    def _forward_G(self, keep_data_for_visuals):
        ...
        fake_img_mask = self._do_if_necessary_saturate_mask(fake_img_mask, saturate=self._opt.do_saturate_mask)
        #print(fake_img_mask.shape,self._real_img.shape)
        ...
        # G(G(Ic1,c2), c1)
        #print("_real_cond",self._real_cond)
        ...
        if keep_data_for_visuals:
            self._vis_real_img = util.tensor2im(self._input_real_img)
            hhh=util.tensor2im(((1 - fake_img_mask) * fake_imgs).data)
            self._vis_fake_img_unmasked = util.tensor2im(fake_imgs.data)
            self._vis_fake_img = util.tensor2im(fake_imgs_masked.data)
            self._vis_fake_img_mask = util.tensor2maskim(fake_img_mask.data)
            self._vis_real_cond = self._input_real_cond.cpu()[0, ...].numpy()
            ...
            filename = os.path.join('/fs1/home/tjuvis_2022/lxx/GANimation-master/png/',
                                    self.count_s+'1img{:03d}.png'.format(self.count_mask),)
            #img = cv2.cvtColor(self._vis_fake_img, cv2.COLOR_RGB2BGR)
            imageio.imwrite(filename, self._vis_fake_img)
            
            filename = os.path.join('/fs1/home/tjuvis_2022/lxx/GANimation-master/png/',
                                    self.count_s+'1__img{:03d}.png'.format(self.count_mask),)
            #img = cv2.cvtColor(hhh, cv2.COLOR_RGB2BGR)
            imageio.imwrite(filename, hhh)
    ...
    def _forward_D(self):
        ...
        #print(d_real_img_cond.shape,self._real_cond.shape)
    ...
    def get_current_errors(self):
        loss_dict = OrderedDict([('g_fake', self._loss_g_fake.data[0]),
                                 ...
                                 ('g_rgb', self._loss_rec_real_img_rgb.data.item()),
                                 ('g_rgb_un', self._loss_g_unmasked_rgb.data.item()),
                                 ('g_rgb_s', self._loss_g_fake_imgs_smooth.data.item()),
