from lip_nopre_load_audface_multiid import load_audface_data
import os
import sys
import numpy as np
import imageio
...
def render_dynamic_face_new(iden_gt_aus,iden,iden_,target,attention_pkl,i_pkl,G_gt_aus,face_real,face_real_,gt_aus,i,H, W, focal, cx, cy, chunk=1024*32, rays=None, bc_rgb=None, aud_para=None,
    ...
    if c2w is None:
        if i % 1000 == 0:
            ...
            rgb8 = to8b(imag_real_mask.cpu().detach().numpy())
            filename = os.path.join('/fs1/home/tjuvis_2022/lxx/NeRF-all/save/pt_Obama_/show/png/', 'mask{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)
            
            #rgb8 = to8b(G1.cpu().detach().numpy())
            #filename = os.path.join('/fs1/home/tjuvis_2022/lxx/NeRF-all/save/pt_Obama_/show/png/', 'G1img{:03d}.png'.format(i))
            #imageio.imwrite(filename, rgb8)
            
            #rgb8 = to8b(iden_imag_real_mask.cpu().detach().numpy())
            #filename = os.path.join('/fs1/home/tjuvis_2022/lxx/NeRF-all/save/May/png/', 'inputimg{:03d}.png'.format(i))
            #imageio.imwrite(filename, rgb8)
            
            rgb8 = to8b(attention_images[0].cpu().detach().numpy())
            filename = os.path.join('/fs1/home/tjuvis_2022/lxx/NeRF-all/save/pt_Obama_/show/png/', 'attention_imagesimg{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)
            
            #rgb8 = to8b(target[0].cpu().detach().numpy())
            #filename = os.path.join('/fs1/home/tjuvis_2022/lxx/NeRF-att/save/png_train_lip_all/hh2', 'targetimg{:03d}.png'.format(i))
            #imageio.imwrite(filename, rgb8)
...
def create_nerf(args):
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        '''
        ckpts_pre=['/fs1/home/tjuvis_2022/lxx/DFRF/save/May/pt/438000_head.tar']
...
def config_parser():
    ...
    parser.add_argument("--expname_finetune", type=str,default='pt_Obama_/show/',
                        help='experiment name')
    ...
    #parser.add_argument("--ft_path", type=str, default='/fs1/home/tjuvis_2022/lxx/NeRF-all/save/May/a/080000_head.tar',
                        help='specific weights npy file to reload for coarse network')
    #parser.add_argument("--ft_path", type=str, default='/fs1/home/tjuvis_2022/lxx/NeRF-att/may_net_epoch_30_id_G.tar',
    #                    help='specific weights npy file to reload for coarse network')
    parser.add_argument("--N_iters", type=int, default=190000,
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            iden_path=face_images[str(select_id)][0]
            ...
            if args.select_nearest:
                ...
                G_gt_aus= G_gt_aus.type(torch.cuda.FloatTensor)
                iden_gt_aus = torch.as_tensor(gt_aus1[str(select_id)][0]).cuda()
                iden_gt_aus= iden_gt_aus.type(torch.cuda.FloatTensor)
        ...
        if global_step >= args.nosmo_iters:
            rgb, disp, acc, _, extras, loss_translation,G1,image_mask_real = render_dynamic_face_new(iden_gt_aus,iden,iden_,target,attention_pkl,i_pkl,G_gt_aus,face_real,face_real_,gt_aus,i,H, W, focal, cx, cy, chunk=args.chunk, rays=batch_rays,
                                                            aud_para=aud_smo, bc_rgb=bc_rgb,
                                                            verbose=i < 10, retraw=True, attention_images = attention_images,attention_images_head = b,attention_images_face = b1,attention_images_face_=attention_images_face_,
                                                            attention_poses = attention_poses, intrinsic = intrinsic, render_pose=pose,lip_rect = lip_rect,head_rect=head_rect, **render_kwargs_train)
            auds_val=aud_smo
        else:
            rgb, disp, acc, _, extras, loss_translation,G1,image_mask_real = render_dynamic_face_new(iden_gt_aus,iden,iden_,target,attention_pkl,i_pkl,G_gt_aus,face_real,face_real_,gt_aus,i,H, W, focal, cx, cy, chunk=args.chunk, rays=batch_rays,
        ...
        if i % 1 == 0:
            ...
            filename = os.path.join('/fs1/home/tjuvis_2022/lxx/NeRF-all/call_lpips/Obama_/test/', 'train{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)
        '''
        optimizer.zero_grad()
        ...
        #mask_loss=img2mse(G1[76:110,22:90,:],image_mask_real[76:110,22:90,:])
        
        trans = extras['raw'][..., -1]
        #print("loss")
        #print(img_loss)
        #print(args.L2loss_weight * loss_translation)
        #print(mask_loss)
        loss = img_loss + args.L2loss_weight * loss_translation#+0.00001*mask_loss
        #loss = img_loss
        psnr = mse2psnr(img_loss)
        ...
        for param_group in optimizer_G.param_groups:
            param_group['lr'] = new_lrate
        for param_group in optimizer_lip.param_groups:
            param_group['lr'] = new_lrate
        ################################
        dt = time.time()-time0
        ...
        if i % args.i_weights == 0:
            ...
            torch.save({
                ...
                'optimizer_lip_state_dict': optimizer_lip.state_dict(),
                'unet_state_dict': render_kwargs_train['feature_extractor'].state_dict(),
                #'unet_state_dict__': render_kwargs_train['feature_extractor2'].state_dict(),
                'unet_state_dict2': render_kwargs_train['generator'].state_dict(),
                'lip_encoder': render_kwargs_train['lip_encoder'].state_dict(),
                #'img_encoder': render_kwargs_train['img_encoder'].state_dict(),

                'attention_state_dict': models['attention_model'].state_dict(),
                #'attention_state_dict2': models['attention_model2'].state_dict(),
                #'attention_state_dict1': models['attention_model1'].state_dict(),
